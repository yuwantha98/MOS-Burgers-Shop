...
const initApp = () => {
  //get data from jason
  fetch("json/products1.json") //this is return a promise
...
document.addEventListener("click", (event) => {
  ...
  if (positionClick.value && positionClick.value.includes("burger")) {
    listProductHTML.innerHTML = "";
    let sortedList = listProducts.filter((item) =>
...
document.getElementById("txtSearch").addEventListener("input", (event) => {
  listProductHTML.innerHTML = "";
  let sortedList = listProducts.filter((item) =>
    item.name.toLowerCase().includes(txtSearch.value.toLowerCase())
  );
  addDataToHTMLCustom(sortedList);
});
